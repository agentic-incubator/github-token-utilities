...
| G16 | There is no authenticated API for revoking your own PAT. `POST /credentials/revoke` accepts `ghp_`, `github_pat_`, `gho_`, `ghu_` and `ghr_` (up to 1,000 per request); authenticated requests return 403; 60 requests/hour; returns 202 (asynchronous); owners are notified; it is irreversible; it is documented for credentials the caller doesn't own | `revokeCredentials`, `waitUntilRevoked`, `revoke.mjs`, `store --revoke-previous` | Documented, Tested here | [REST: revocation](https://docs.github.com/en/rest/credentials/revoke); `tests/cli.test.mjs` runs against a fake endpoint and checks that no `Authorization` header is sent, the body shape, and the 202/403 handling |
