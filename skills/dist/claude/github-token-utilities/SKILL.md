...
- **Asking the user:** Use the `AskUserQuestion` tool for choices and confirmations (up to 4 questions at once, each with 2-4 options).
- **Running commands:** Use the `Bash` tool.
- **Commands the user must run themselves:** Ask the user to run the command in a separate terminal window. Don't suggest Claude Code's `!` prefix for these: the hidden token prompt needs an interactive terminal.
