...
## How to work in OpenAI Codex CLI

- **Asking the user:** Ask in plain text, number the options, and wait for the reply before continuing. (`request_user_input` is only available in Plan mode, so don't depend on it.)
- **Running commands:** Use your shell tool (`exec_command`). Commands in this skill are non-interactive, except those you hand to the user.
