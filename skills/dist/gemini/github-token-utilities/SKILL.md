...
## How to work in Gemini CLI

- **Asking the user:** Use the `ask_user` tool (choice, yesno or text questions; up to 4 at once).
- **Running commands:** Use the `run_shell_command` tool.
- **Commands the user must run themselves:** Ask the user to run the command in a separate terminal window, then tell you when it finishes. Gemini CLI's `!` shell mode is interactive, so running it there also works; the token prompt is hidden and the script never prints the token.
