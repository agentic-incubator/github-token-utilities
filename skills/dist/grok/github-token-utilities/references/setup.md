...
1. **Explain and confirm.** Tell the user exactly what will happen, then ask to proceed:
   - clone `https://github.com/agentic-incubator/github-token-utilities` into the location above
...
2. **Clone** (or update, if the folder already exists):
   ...
   if [ -d "$DIR/.git" ]; then git -C "$DIR" pull --ff-only; else git clone --depth 1 https://github.com/agentic-incubator/github-token-utilities.git "$DIR"; fi
   ```
   ...
   if (Test-Path (Join-Path $Dir '.git')) { git -C $Dir pull --ff-only } else { git clone --depth 1 https://github.com/agentic-incubator/github-token-utilities.git $Dir }
   ```
